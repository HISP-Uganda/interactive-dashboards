...
export const useInitials = () => {
    ...
    const ouQuery = {
        ...
        dataElementGroupSets: {
            resource:
                "dataElementGroupSets.json?filter=attributeValues.attribute.id:eq:MeGs34GOrPw&fields=dataElementGroups[id,name,code,dataElements[id,code,name,attributeValues[value,attribute[id,name]],dataElementGroups[id,groupSets[id,attributeValues[value,attribute[id,name]]]]]],id,code,name,attributeValues[attribute[id,name],value]",
        },
        directives: {
            resource:
    ...
    return useQuery<any, Error>(
        ...
        async ({ signal }) => {
            ...
            const processedDirectives = dataElementGroups.flatMap(
                ({
                    ...
                    return dataElements.map(
                        ({ id, name, code, attributeValues: deav }: any) => {
                            let programCode = "";
                            ...
                            const isDecreasing = decreasing && decreasing.value;
                            return {
                                id,
                                name,
                                code,
            ...
            setDecreasing(
                [...processedGroupSets, ...processedDirectives]
                    .filter(({ isDecreasing }: any) => isDecreasing)
                    .map(({ id }) => id)
            );
            updateVisualizationData({
