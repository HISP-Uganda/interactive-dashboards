                ...
                <Td>
                  <NumberInput
                    ...
                    onChange={(valueString) => {
                      setUserInput2(() => Number(valueString));
                    }}
                  >
                    <NumberInputField />
                    <NumberInputStepper>
              ...
              <Stack direction="row">
                ...
                <Td>
                  <NumberInput
                    ...
                    max={10}
                    onChange={(valueString) => {
                      setUserInput3(Number(valueString.replace(/,/g, "")) || 0);
                    }}
                  >
                    <NumberInputField />
                    <NumberInputStepper>
        ...
        <Tbody>
          ...
          <Tr fontWeight="semibold">
            ...
            <Td border="1px solid black">
              <NumberInput
                ...
                max={12}
                onChange={(valueString) => {
                  setUserInput(Number(valueString.replace(/,/g, "")) || 0);
                }}
              >
                <NumberInputField />
                <NumberInputStepper>
            ...
            <Td border="1px solid black">
              <NumberInput
                ...
                max={12}
                onChange={(valueString) => {
                  setUserInput1(Number(valueString.replace(/,/g, "")) || 0);
                }}
              >
                <NumberInputField />
                <NumberInputStepper>
          ...
          {processedData.finalData.map((orgUnit) => {
            ...
            const totalPopulationLiveBirths =
              Math.floor(
                ((Number(orgUnit["eHKadEd89Np"].replace(/,/g, "")) * 0.0485) /
                  12) *
                  userInput
              ) || 0;
            ...
            const totalPopulationBelowOneYear =
              Math.floor(
                ((Number(orgUnit["eHKadEd89Np"].replace(/,/g, "")) * 0.043) /
                  12) *
                  userInput1
              ) || 0;

            const childrenImmunizedBCG1 = Math.floor(orgUnit["qxXm4iCRELh"]);
            const childrenImmunizedDPT1 = Math.floor(orgUnit["TY3Nmc0EL3U"]);
            const childrenImmunizedDPT3 = Math.floor(orgUnit["t2CpG3vv7sO"]);
            const childrenImmunizedMR1 = Math.floor(orgUnit["Rq3LICjHeiM"]);
            const coverageBCG = (
              (childrenImmunizedBCG1 / totalPopulationLiveBirths) *
              100
            ).toFixed(2);
            const coverageDPT1 = (
              (childrenImmunizedDPT1 / totalPopulationBelowOneYear) *
              100
            ).toFixed(2);
            const coverageDPT3 = (
              (childrenImmunizedDPT3 / totalPopulationBelowOneYear) *
              100
            ).toFixed(2);
            const coverageMR1 = (
              (childrenImmunizedMR1 / totalPopulationBelowOneYear) *
              100
            ).toFixed(2);
            const zeroDose =
            ...
            const unImmunizedMR1 = childrenImmunizedDPT1 - childrenImmunizedMR1;
            ...
            const dropOutRateDPT1ToMR1Numeric = parseFloat(coverageDPT1);
            const access =
              dropOutRateDPT1ToMR1Numeric > userInput2 ? "Good" : "Poor";
            ...
            //DropOut Legend
            const getDropOutRateBgColor = (value: number): string => {
            ...
            const bgColor = getDropOutRateBgColor(dropOutRateDPT1ToDPT3Value);
            ...
            if (zeroDoseValue >= -100000 && zeroDoseValue < 100) {
              zeroBgColor = "#FECC5C";
            } else if (zeroDoseValue >= 100 && zeroDoseValue < 2000) {
              zeroBgColor = "#FD8D3C";
              color = "black";
            } else if (zeroDoseValue >= 2000 && zeroDoseValue < 100000) {
              zeroBgColor = "#BD0026";
            } else {
              zeroBgColor = "#DBDBDB";
            }
            return (
              <Tr key={orgUnit["ou-name"]}>
                ...
                <Td border="1px solid black">{totalPopulation}</Td>
                <Td border="1px solid black">{totalPopulationLiveBirths}</Td>
                <Td border="1px solid black">{totalPopulationBelowOneYear}</Td>
                <Td border="1px solid black">{childrenImmunizedBCG1}</Td>
                <Td border="1px solid black">{childrenImmunizedDPT1}</Td>
                <Td border="1px solid black">{childrenImmunizedDPT3}</Td>
                <Td border="1px solid black">{childrenImmunizedMR1}</Td>
                <Td border="1px solid black" bgColor={bgColors[0]}>
                  {coverageBCG}%
                </Td>
                <Td border="1px solid black" bgColor={bgColors[1]}>
                  {coverageDPT1}%
                </Td>
                <Td border="1px solid black" bgColor={bgColors[2]}>
                  {coverageDPT3}%
                </Td>
                <Td border="1px solid black" bgColor={bgColors[3]}>
                  {coverageMR1}%
                </Td>
                <Td border="1px solid black" bgColor={zeroBgColor}>
                  {zeroDose}
                </Td>
                <Td border="1px solid black">{unImmunizedDPT3}</Td>
                <Td border="1px solid black">{unImmunizedMR1}</Td>
                ...
                <Td
                  ...
                  fontSize="xl"
                  bgColor={access === "Good" ? "green.600" : "red"}
                  color={access === "Good" ? "white" : "white"}
                  fontWeight={access === "Good" ? "extrabold" : "extrabold"}
                >
                  {access}
                </Td>
                <Td
                  border="1px solid black"
                  fontSize="xl"
                  bgColor={utilization === "Good" ? "green.600" : "red"}
                  color={utilization === "Good" ? "white" : "white"}
                  fontWeight={
                    utilization === "Good" ? "extrabold" : "extrabold"
                  }
                >
                  {utilization}
                </Td>
                <Td
                  ...
                  bgColor={getCategoryBgColor(categoryValue())}
                >
                  {categoryValue()}
                </Td>
                <Td
