...
const PeriodPicker = ({ selectedPeriods, onChange }: PickerProps) => {
  ...
  //     console.log("From: ", dates[0], ", to: ", dates[1]);
  //     console.log("From: ", dateStrings[0], ", to: ", dateStrings[1]);
  //     // Create a new period object for the selected date range
  //     const newPeriod: Period = {
  //       label: `${dateStrings[0]} to ${dateStrings[1]}`, // Customize the label as needed
  //       value: [dates[0], dates[1]], // Store the date range values
  //       type: "dateRange", // Add a type to differentiate from fixed and relative periods
  //     };
  //     // Update the selected periods state with the new period
  //     onChange([...selectedPeriods, newPeriod], false);
  //   } else {
  //     console.log("Clear");
  //   }
  // };
  ...
  return (
    <Stack direction="row" p="2px" w="800px">
          ...
          <TabPanels>
            <TabPanel>
              <Stack spacing="20px">
                  ...
                  <Select<Option, false, GroupBase<Option>>
                    ...
                    onChange={(e) =>
                      setRelativePeriodType(
                        () => (e?.value as RelativePeriodType) || "MONTHLY"
                      )
                    }
                    value={relativePeriodTypeOptions.find(
                      ({ value }) => value === relativePeriodType
                    )}
                ...
                <Stack>
                  {availableRelativePeriods.map(({ label, value }) => (
                    <Text
                      key={value}
                      cursor="pointer"
                      onClick={() =>
                ...
                <Stack direction="row">
                  <Stack flex={1}>
                    ...
                    <Select<Option, false, GroupBase<Option>>
                      ...
                      onChange={(e) => {
                        setFixedPeriodType(
                          () => (e?.value as FixedPeriodType) || "MONTHLY"
                        );
                      }}
      ...
      <Stack w="48px" alignItems="center" justifyContent="center" spacing={1}>
        <IconButton
              ...
              <Stack overflow="auto">
                {selectedPeriods.map(({ value, label }) => (
                  <Text
                    ...
                    onClick={() =>
                      onChange(
                        selectedPeriods.filter((v) => v.value === value),
                        true
                      )
                    }
                  >
