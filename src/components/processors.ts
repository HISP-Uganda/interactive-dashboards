...
export const findColor = (val: number, thresholds: Threshold[]) => {
  ...
  const baseline = thresholds.find(({ id }) => id === "baseline")?.color || "";
          ...
          for (const p of currentValues) {
            nextValues = [
              ...nextValues,
              {
                ...
                span: 1,
                actual: p.value,
                position: properties?.[`${p.value}.position`] || 1,
              },
            ];
          }
        }
...
export const getNData = (data: any[], values: string[], index: number) => {
  if (data) {
    ...
    const specialized = values.filter((d) => SPECIAL_COLUMNS.indexOf(d) !== -1);
      ...
      .flatMap(([key, values]) => {
        ...
        const normal = Object.entries(groupedByColumn).map(
          ([columnKey, columnData]) => {
            return {
              key: `${key}${columnKey}`,
              value: calculation[aggregation](columnData, thresholds, {
                aggregationColumn,
...
export const processSingleValue = (
  ...
  if (processed.length > 0) {
    if (aggregate && aggregationColumn) {
      ...
      if (aggregationStrategy && aggregationStrategyColumn) {
        const current = Object.entries(processed).flatMap(
          ([_, val]: [string, any]) => {
            ...
            if (aggregationStrategy === "last") return val[val.length - 1];
      ...
      if (processed.length === 1 && Object.keys(processed[0]).length === 1) {
        return values[0];
      }
      if (processed.length === 1 && Object.keys(processed[0]).length > 1) {
...
export const processGraphs = (
  ...
  if (data && data.length > 0 && options.category !== undefined) {
    if (options.order) {
      data = orderBy(data, "value", [options.order as "asc" | "desc"]);
    }
    if (options.show) {
      data = data.slice(0, options.show);
    }
    ...
    if (options.summarize) {
      ...
      if (options.series) {
        allSeries = uniq(data.map((num: any) => num[options.series || ""]));
  ...
  return [
    {
      ...
      textfont: {
        size: [16, 16, 16],
        color:
          colors.length > 0 ? colors.map((c) => invertHex(c, true)) : undefined,
      },
      // hole: 0.1,
      marker: {
